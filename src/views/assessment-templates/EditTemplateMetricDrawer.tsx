...
const EditTemplateMetricDrawer = ({ open, templateMetric, metrics, dataSources, handleClose, onUpdated }: Props) => {
  const {
    control,
    reset,
    handleSubmit,
  ...
  useEffect(() => {
    if (templateMetric) reset({ metricId: templateMetric.metricId, sourceId: templateMetric.sourceId })
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const res = await fetch('/api/template-metrics', {
      ...
      body: JSON.stringify({ id: templateMetric.id, templateId: templateMetric.templateId, ...data })
    })
