...
type FormData = { metricId: number }

const EditSessionMetricDrawer = ({ open, sessionMetric, metrics, handleClose, onUpdated }: Props) => {
  const { control, reset, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormData>({ defaultValues: { metricId: 0 } })

  useEffect(() => {
    if (sessionMetric) reset({ metricId: sessionMetric.metricId })
  }, [sessionMetric, reset])
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const res = await fetch('/api/session-metrics', {
      ...
      body: JSON.stringify({ id: sessionMetric.id, session1Id: sessionMetric.session1Id, metricId: data.metricId })
    })
