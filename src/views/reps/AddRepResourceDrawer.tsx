...
type Props = {
  ...
  handleClose: () => void
  onCreated: (r: RepResourceType) => void
}

type FormData = { type: string; url: string }

const AddRepResourceDrawer = ({ open, repId, resourceTypes, handleClose, onCreated }: Props) => {
  ...
  const onSubmit = async (data: FormData) => {
    const res = await fetch('/api/rep-resources', {
      ...
      body: JSON.stringify({ repId, ...data })
    })
  ...
  return (
    <Drawer open={open} anchor='right' variant='temporary' onClose={handleReset} ModalProps={{ keepMounted: true }} sx={{ '& .MuiDrawer-paper': { width: { xs: 300, sm: 400 } } }}>
      <div className='flex items-center justify-between pli-5 plb-4'>
        <Typography variant='h5'>Add Rep Resource</Typography>
...
export default AddRepResourceDrawer
