...
type Props = {
  ...
  handleClose: () => void
  onUpdated: (report: ReportType) => void
}
...
const EditReportDrawer = ({ open, report, handleClose, onUpdated }: Props) => {
  const {
    control,
    reset,
    handleSubmit,
  ...
  useEffect(() => {
    if (report) reset({ name: report.name, extRef: report.extRef ?? '' })
  }, [report, reset])

  const onSubmit = async (data: FormData) => {
    if (!report) return

    const body: Record<string, unknown> = { id: report.id, name: data.name }
